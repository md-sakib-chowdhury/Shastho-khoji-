...
@media(max-width:767px) {
    .h-popup-box { max-width:300px; padding:20px 16px; border-radius:16px; }
}
/* Tablet ── 768–1023px */
@media(min-width:768px)and(max-width:1023px) {
    .h-popup-box { max-width:400px; padding:36px 28px; }
}
/* Laptop 14" ── 1024–1279px */
@media(min-width:1024px)and(max-width:1279px) {
    .h-popup-box { max-width:400px; padding:40px 32px; }
}
/* Laptop 15" ── 1280–1439px */
@media(min-width:1280px)and(max-width:1439px) {
    .h-popup-box { max-width:420px; padding:42px 34px; }
}
/* Desktop ── 1440px+ */
@media(min-width:1440px) {
    .h-popup-box { max-width:440px; padding:44px 36px; }
}
`;
...
function WelcomeVoicePopup({ onClose }) {
    const speak = () => {
        ...
        const u = new SpeechSynthesisUtterance(
            "Welcome to Swasthya Khoji. Find your preferred doctor easily."
        );
        u.lang = "en-US"; u.rate = 0.9; u.pitch = 1.05;
        const v = window.speechSynthesis.getVoices().find(x => x.lang === "en-US") || null;
        if (v) u.voice = v;
    ...
    return (
        <div className="h-popup-ov">
            <div className="h-popup-box">
                <div style={{ fontSize: "clamp(30px,7vw,44px)", marginBottom: 8 }}>🏥</div>
                <h2 style={{
                    fontFamily: "'Noto Serif Bengali',serif",
                    fontSize: "clamp(14px,3.5vw,20px)",
                    color: "#0d3b1e", margin: "0 0 8px",
                }}>স্বাস্থ্য খোঁজিতে স্বাগতম!</h2>
                <p style={{
                    color: "#5a7a66",
                ...
                <button
                    ...
                    style={{
                        ...
                        boxShadow: "0 6px 20px rgba(46,158,86,.35)",
                    }}
                >🔊 শুরু করুন</button>
                <button
                    onClick={onClose}
...
function HowItWorks() {
    ...
    useEffect(() => {
        const obs = new IntersectionObserver(entries => {
            entries.forEach(e => {
                if (!e.isIntersecting) return;
                stepsRef.current.forEach((el, i) => {
                    if (el) setTimeout(() => { el.style.opacity = "1"; el.style.transform = "translateY(0)"; }, i * 150);
                });
                obs.unobserve(e.target);
            });
        }, { threshold: 0.2 });
    ...
    return (
        <div ref={secRef} style={{ marginBottom: 72 }}>
            <SectionHeader badge="কিভাবে কাজ করে" title="মাত্র ৩টি সহজ ধাপে"
                subtitle="শুরু থেকে চিকিৎসা পর্যন্ত — সবকিছু এক জায়গায়" />
            <div className="h-how-grid">
                <div className="h-how-line" />
                {steps.map((s, i) => (
                    <div key={s.step} ref={el => stepsRef.current[i] = el}
...
function Testimonials() {
        ...
        const obs = new IntersectionObserver(entries => {
            entries.forEach(e => {
                ...
                cardRefs.current.forEach((el, i) => {
                    if (el) setTimeout(() => { el.style.opacity = "1"; el.style.transform = "translateY(0) scale(1)"; }, i * 100);
                });
                obs.unobserve(e.target);
            });
        }, { threshold: 0.1 });
    ...
    return (
        <div ref={secRef} style={{ marginBottom: 72 }}>
            <SectionHeader badge="রোগীর কথা" title="তারা কী বলছেন"
                subtitle="হাজারো সন্তুষ্ট রোগীর মধ্যে কিছু অভিজ্ঞতা" />
            <div className="h-test-grid">
                {testimonials.map((t, i) => (
                    <div key={t.name} ref={el => cardRefs.current[i] = el}
                        style={{
    ...
    useEffect(() => {
        const obs = new IntersectionObserver(entries => {
            entries.forEach(e => {
                if (!e.isIntersecting) return;
                cardRefs.current.forEach((el, i) => {
                    if (el) setTimeout(() => { el.style.opacity = "1"; el.style.transform = "translateY(0)"; }, i * 120);
                });
                obs.unobserve(e.target);
            });
        }, { threshold: 0.1 });
            ...
            <div className="h-doc-grid">
                {featuredDoctors.map((d, i) => (
                    ...
                    >
                        ...
                        <div style={{ fontSize: 11, color: "#5a7a66", marginBottom: 14 }}>অভিজ্ঞতা: {d.exp}</div>
                        <Link to={`/search?specialization=${d.spec}`} style={{ textDecoration: "none" }}>
                            <div style={{ background: d.color, color: "#fff", borderRadius: 50, padding: "9px 16px", fontSize: 12, fontWeight: 700, boxShadow: `0 4px 14px ${d.color}40` }}>
                                অ্যাপয়েন্টমেন্ট নিন
                            </div>
                        </Link>
                    </div>
